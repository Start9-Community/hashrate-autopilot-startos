...
**Dual-chain caveat since the 8/8/2026 BIP110 split (#363).** Everything above describes the *mainstream* chain's API. Since block 961,632 Ocean runs two chains with fully separate TIDES accounting ("dual-TIDES" per Ocean's own announcements; Lightning payouts are paused while it lasts). Empirical probes (2026-08-17): a miner pointed at `bip110.mine.ocean.xyz:3110` (or a BIP110 node + DATUM) is invisible on `api.ocean.xyz` - `statsnap`/`user_hashrate` return "No such user" - and NO JSON API exists for the BIP110 side: no `api.bip110.ocean.xyz` DNS, `/v1/*` on `bip110.ocean.xyz` 404s, and `api.ocean.xyz` ignores every chain-selection query param probed. The only machine-readable source is `bip110.ocean.xyz`'s own HTML template fragments (`/template/workers/*`, `/template/poolstatus`, `/dashboard?bpage=N`) - the same fragment endpoints this project scraped from `ocean.xyz` before issue #9. `packages/daemon/src/services/ocean-bip110.ts` revives that scraper for the BIP110 sharelog, selected via the `ocean_chain` config key; its per-field sources and degradations are documented in the module header. The BIP110 blocks list includes the shared pre-fork history, which is what lets the pool-blocks backfill terminate naturally on its lookback cutoff after a chain flip.
