...
**Dual-chain caveat since the 8/8/2026 BIP110 split (#363).** Everything above describes the *mainstream* chain's API. Since block 961,632 Ocean runs two chains with fully separate TIDES accounting ("dual-TIDES" per Ocean's own announcements; Lightning payouts are paused while it lasts). Empirical probes (2026-08-17): a miner pointed at `bip110.mine.ocean.xyz:3110` (or a BIP110 node + DATUM) is invisible on `api.ocean.xyz` - `statsnap`/`user_hashrate` return "No such user" - and NO JSON API exists for the BIP110 side: no `api.bip110.ocean.xyz` DNS, `/v1/*` on `bip110.ocean.xyz` 404s, and `api.ocean.xyz` ignores every chain-selection query param probed. The site's HTML template fragments (`/template/workers/*`, `/template/poolstatus`, `/dashboard?bpage=N` - the same endpoints this project scraped before issue #9) do render BIP110 per-address data, and a scraper-based client was briefly built on them (build 817, develop only, never released). **Withdrawn 2026-08-18 after asking Ocean directly.** Their support's answer, verbatim: "There is no API for the BIP 110 chain, nor any plan to develop one. Scraping is also not supported. So I'm afraid there is no way for you to retrieve the data for hashrate bot." So the `ocean_chain` config key is now purely declarative: on `bip110` the daemon stops polling Ocean (a chain-gated client short-circuits every call), the dashboard explains the gap instead of showing zeros, and the hashprice-dependent dynamic cap is bypassed. On-chain payout observation via the operator's own node keeps working - their node follows their chain. Follow-ups: P&L `collected` on bip110 is derived from that on-chain ledger (`reward_events`) since earnpay can never sync there, with Lightning explicitly untrackable (#366); and no hashprice is served or derived on bip110 (Braiins spend is mainstream sats, BIP110 earnings are chain-local sats - not comparable), so the dynamic cap and cheap mode are inert and the dashboard marks every Ocean/hashprice surface as n/a while still plotting pre-switch history (#367, #368). The user-facing summary lives in the README chapter "Mining on Ocean's BIP110 chain".
