...
On `bip110` there is **no hashprice** (#367): Ocean provides none and none is derived - a calculated value would price BIP110-chain sats against the mainstream sats spent on Braiins, which are not comparable assets. Consequently the dynamic price cap (`max_overpay_vs_hashprice_sat_per_eh_day`) and cheap mode (`cheap_*`) are inert; the Config screen renders both disabled with the reason (saved values are kept and apply again on `mainstream`), the dashboard drops every hashprice-derived row (avg hashprice, projected income/day, net/day, Ocean's income estimate, bid-vs-hashprice, the "max over hashprice"/"effective cap" rows and the chart's cap overlay), and the daemon stops serving hashprice on that chain entirely - including suppressing the stale pre-flip cache value that would otherwise leak into the status route and tick metrics forever. The fixed `max_bid` is the only price ceiling.
