...
On `bip110` there is **no hashprice** (#367): Ocean provides none and none is derived - a calculated value would price BIP110-chain sats against the mainstream sats spent on Braiins, which are not comparable assets. Consequently the dynamic price cap (`max_overpay_vs_hashprice_sat_per_eh_day`) and cheap mode (`cheap_*`) are inert; the Config screen renders both disabled with the reason (saved values are kept and apply again on `mainstream`), the dashboard drops the hashprice-derived P&L rows (avg hashprice, projected income/day, net/day, Ocean's income estimate) and the "max over hashprice"/"effective cap" rows plus the chart's cap overlay, and the daemon stops serving hashprice on that chain entirely - including suppressing the stale pre-flip cache value that would otherwise leak into the status route and tick metrics forever. Tiles, the tile picker, and the chart right-axis dropdowns mark dead Ocean/hashprice entries as n/a instead of hiding them (#368) - they stay selectable so pre-switch mainstream history keeps plotting, with the affected chart legend chips dimmed. The fixed `max_bid` is the only price ceiling.
